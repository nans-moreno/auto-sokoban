...
class SokobanCompleteGame:
    ...
    def create_buttons(self):
        ...
        # Boutons en jeu
        self.game_buttons = [
            Button(700, 150, 120, 40, "Annuler (U)", self.small_font),
            Button(700, 200, 120, 40, "Recommencer (R)", self.small_font),
            Button(700, 250, 120, 40, "Menu (M)", self.small_font),
            Button(700, 300, 120, 40, "Scores", self.small_font)
        ]
    ...
    def draw_game(self):
        ...
        for i, instruction in enumerate(instructions):
            ...
            self.screen.blit(text, (700, 350 + i * 20))
    ...
    def load_assets(self):
        ...
        try:
            ...
            # Caisse sur cible (utilise une image différente ou la même avec un effet)
            self.assets[4] = pygame.image.load("assets/box.png")
            self.assets[4] = pygame.transform.scale(self.assets[4], (CELL_SIZE, CELL_SIZE))
            
            # Joueur sur cible
            self.assets[5] = pygame.image.load("assets/player_on_target.png")
            self.assets[5] = pygame.transform.scale(self.assets[5], (CELL_SIZE, CELL_SIZE))
            
        except Exception as e:
            print(f"⚠️ Erreur lors du chargement des assets: {e}")
            # Créer des surfaces de couleur par défaut si les images ne se chargent pas
            self.create_fallback_assets()
    
    def create_fallback_assets(self):
        """Crée des assets de secours si les images ne peuvent pas être chargées"""
        # Couleurs de secours
        colors = {
        ...
        for value, color in colors.items():
            ...
            surface.fill(color)
            # Ajouter une bordure pour mieux distinguer les éléments
            pygame.draw.rect(surface, (0, 0, 0), surface.get_rect(), 1)
            self.assets[value] = surface
